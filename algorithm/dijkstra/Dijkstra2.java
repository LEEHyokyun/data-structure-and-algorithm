...
public class Dijkstra2 {
    ...
    public Dijkstra2(HashMap<String, WeightedGraphCityVertex> allCities) {
    ...
    public void Dijkstra(WeightedGraphCityVertex startCityVertex, WeightedGraphCityVertex endCityVertex){
        ...
        HashMap<String, AdditionalInfo> dijkstraTable = new HashMap<>();
        ...
        for(WeightedGraphCityVertex cityVertex: unvisitedCities.values()){
            dijkstraTable.put(cityVertex.getName(), new AdditionalInfo(Double.POSITIVE_INFINITY));
        }

        dijkstraTable.get(startCityVertex).setDistance(0D);
        ...
        while(!unvisitedCities.isEmpty()){
             ...
             * */
            for(WeightedGraphCityVertex cityVertex: unvisitedCities.values()){
                if(currentCityVertex == null || dijkstraTable.get(cityVertex.getName()).getDistance() < dijkstraTable.get(currentCityVertex.getName()).getDistance()){
            ...
            for(String adjacentCity : visitedCities.get(currentCityVertex.getName()).getAdjacenctCities().keySet()){
                ...
                Double distance = dijkstraTable.get(currentCityVertex.getName()).getDistance() + visitedCities.get(currentCityVertex.getName()).getAdjacenctCities().get(adjacentCity);

                if(dijkstraTable.get(adjacentCity).getDistance() > distance){
                    dijkstraTable.get(adjacentCity).setDistance(distance);
                    dijkstraTable.get(adjacentCity).setPathInfo(visitedCities.get(currentCityVertex).getName());
                }
            }
        }
