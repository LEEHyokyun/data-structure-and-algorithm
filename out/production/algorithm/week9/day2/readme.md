...
- 땅따먹기
  - DFS로 접근? row = 4이지만 열의 갯수가 100000, 총 가능한 경우의 수는 
  - DFS는 완전탐색의 일환으로, row ^ col이 100만개 이상? DP을 의심하자.
    - 핵심은 정방향이 아니라 역방향, 이미 이전에서 최대값 연산을 마쳤다고 생각하여, 이 중 최대값을 선택하는 방법.
    - col = j 일때, 이전 row의 col값들에 대해서 col = j인 경우를 제외한 항목만 추출해야 한다면?
      - j+1 % 4, j+2 % 4, j+3 % 4
