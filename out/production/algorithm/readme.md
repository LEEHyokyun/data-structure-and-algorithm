...
> 간선에 크기를 저장하는 그래프.
- 정점간 거리 혹은 정점간 비용을 저장할 수 있다.
- 방향그래프에서 방향에 따라 가중치는 달라질 수 있다.

## 23. 다익스트라 알고리즘

> 가장 빠른 경로를 찾는 대표적인 최단경로 알고리즘 중 하나.
- 출발지점을 지정한 후, 그 지점의 인접노드 중 가중치 합이 가장 작은 노드로 이동해가면서 출발지점에서 모든 지점으로의 최단 경로를 구하는 알고리즘이다.
- 출발지점은 항상 고정이며, 방문 후 인접노드들의 가중치 합을 계산해나간다.
- 가중치 합을 계산한 이후, 가중치 테이블에서 출발지점 기준 가장 그 값이 작은 노드를 다음 방문지로 지정한다.

## 24. 탐욕 알고리즘

> Greedy, 매 선택 시 특정한 기준으로, 가장 좋은(최적의) 선택지를 선택하여 최적의 해를 구하는 방법.
- 매트로이드 문제 상황에 대한 접근방법.
- 탐욕스러운 선택 속성(앞의 선택이 뒤 선택에 영향을 주지 않는다) 및 최적 부분 구조 조건(문제의 최적해가 부분 문제에서도 최적해이다)을 모두 만족할때 탐욕알고리즘을 활용하여 최적의 해를 도출할 수 있다.
- 쉽게 말하면 매순간의 특정 기준에 따른 선택은 최종적으로 최적의 해 도출을 보장할 수 있다.
- 각 순간의 선택 혹은 그 선택의 기준은 변하지 않는다.

## 25. 최소신장트리

> Minimum Spanning Tree(MST), 모든 정점들을 순회하지 않고 가중치의 합이 가장 작은 그래프(트리) 자료구조.
- E = V-1
- 프림 알고리즘 및 크루스칼 알고리즘을 활용하여 구현, 무방향 그래프에서 각 인접 노드 간의 최소 가중치를 찾는 알고리즘이다.
- 다익스트라와 유사하지만, 다익스트라는 최초지점에서 각 정점으로 도착하는 최단 경로를 찾는 알고리즘이라는 점에서 차이가 있다.

## 26. 최대유량문제

> Network flow, 각 정점간에 흐를 수 있는 용량이 정의되어 있을때 최대로 흘려보낼 수 있는 유량의 총합을 도출하는 알고리즘 문제이다.
...
[개념]
> 비트연산, 기존의 원시자료형(Int - 32bit/boolean - 8bit)보다 더 적은 메모리로 빠른 연산을 기대할 수 있는 비트활용방법.
- AND*논리곱연산(A&B)/OR*논리합연산(A|B)/XOR*배타합연산(A^B)/왼쪽시프트*레스댄연산(A<<1)/오른쪽시프트*그레이터댄연산(A>>1)

[외판원 문제]
> TSP(Traveling Salesman Problem), 최소 비용의 경로를 추출하는 NP-Hard 문제.
- 여러 노드들이 존재하고 각 노드로 향하는 간선과 그 비용이 나타나있을때, 최초 노드부터 시작하여 모든 노드를 방문하고 다시 최초 노드로 돌아오는 경로 중 최소비용의 경로를 구하는 문제.
- DP중 메모이제이션을 활용하여 기존 O(N^N)의 시간복잡도를 O(N^N*2^N)까지 개선할 수 있다.
...
- 조합 : 순서상관없다.
  - 중복을 허용하지 않는 일반적인 조합 : i = start, start는 선택한 단계에서 다음 단계로(i+1)
  - 중복을 허용한다(보통 각 동전의 개수 등이 주어진 상황) : i = start, 시작점은 항상 해당 단계에서 고정 유지
  - 중복의 핵심은 하나 고르고, 그 고른 케이스는 제외한다는 것(다음 단계로 넘어가는 것이 포인트) 
  - 다음단계로 바로 넘어가느냐 -> selected 필요 없음, 
  - 조합의 제어 포인트는 항상 depth, 다음 단계로 넘어가기에 visited 방문여부 필요 없음.
- 순열 : 순서상관있다.
  - 기본적으로 항상 i = 0 , 시작점 첫번째부터 고정, 다만 selected를 사용하여 한번 고른 문자열에 대한 중복선택은 방지
  - 단, 중복을 허용하는 순열의 경우 i = 0 시작점은 고정하지만, dfs의 depth가 제어 포인트.

```scss
조합  → start로 중복 제거
순열  → selected로 중복 제거
start는 "뒤만 보게 하는 장치"
selected는 "이미 썼는지 기억하는 장치"
```
...
- 배열의 크기가 소수인데 무제한 사용 가능? 방문여부가 의미가 없다?
  - DP가 확실.