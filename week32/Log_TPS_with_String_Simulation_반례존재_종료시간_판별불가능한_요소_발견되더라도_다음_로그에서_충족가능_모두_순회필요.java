...
public class Log_TPS_with_String_Simulation_반례존재_종료시간_판별불가능한_요소_발견되더라도_다음_로그에서_충족가능_모두_순회필요 {
    static class Log{
    ...
    public int solution(String[] lines) {
        ...
        for(int i = 0 ; i < logs.length ; i++){
            ...
            for(int j = i ; j < logs.length ; j++){
                ...
                //종료시간 기준으로 정렬되었지만
                //한 요소가 충족하지 못하더라도, 로그의 길이가 충분히 길어 다음 로그에서 판별 가능할 수 있음
                //즉, break가 아닌 continue로 계속 탐색이 필요하다.
